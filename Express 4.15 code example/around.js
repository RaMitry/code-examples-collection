...
exports = module.exports = function (req, res) {
    ...
    function getVotes(objects_list, next, long, lat) {
        ...
        if (typeof req.query.location !== "undefined" && req.query.location !== "") {
          ...
          long = parseFloat(location_req[0]);
          lat = parseFloat(location_req[1]);
        }

        if (typeof shifted_obj.loc !== "undefined" && shifted_obj.loc !== "") {
          shifted_obj.distance = keystone.utils.kmBetween([long, lat], shifted_obj.loc.geo) * 1000;
        }        
        ...
        qv.exec(function (err, result) {
            ...
            if (objects_list.length > 0) {
                getVotes(objects_list, next, long, lat);
            } else {
                locals.data.sort(function(a, b) {
    ...
    function sortParameters() {
        ...
        locals.sort_parameters = [];
        locals.sort_parameters_obj = {};
    ...
    function generatePaginationObject(){
        ...
        if (locals.sort_parameters.length > 0) {

            return keystone.list('Object').paginate({
    ...
    function getRequestedObjectPagesAndRender() {
        view.on('init', function (next) {
            ...
            var q = generatePaginationObject();
        ...
        view.render('dashboard-new/static-pages/around.ejs');
